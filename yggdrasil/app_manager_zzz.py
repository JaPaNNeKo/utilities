...
# TODO Put directly in classes?
AppLocal = namedtuple("AppLocal", ('name', 'path_project', 'version_py', 'entry_point', 'env'))
AppWeb = namedtuple("AppWeb", ('name', 'version_py', 'url'))
...
class AppManagerGeneric(object):
    """
    Class managing the creation, deletion and update of 'app'.
    ...
    def from_root(cls, root: str):
        return AppManagerGeneric(apps=cls._get_apps('{0}\settings\settings.txt'.format(root)), root=root)
    ...
    #     cmds = []
    #     cmds.append('workon {0} & setprojectdir "{1}"'.format(self.app[name].env,
    #                                                                      self.app[name].path_project))
    #     cmds.append(
    #         r'workon {1} & pip install --trusted-host pypi.org --trusted-host files.pythonhosted.org -r "{0}\requirements.txt"'
    #         .format(self.app[name].path_project, self.app[name].env))
    #     _run_cmds(cmds)
    ...
    def rm_app(self, name: str, **kwargs):
        ...
        if nb_venv_uses == 1:
            _run_cmds(['rmvirtualenv {0}'.format(self.apps[name].env)])
...
class AppManagerLocal(AppManagerGeneric):
    _replacements = [
        ('#name_venv#', 'env'),
        ('#entry_point#', 'entry_point'),
    ]
    ...
    @classmethod
    def _get_apps(cls, path_settings: str):
        settings = _parse_settings(path_settings)
        return {
            elt['name']:
                AppLocal(
                    name=elt['name'],
...
class AppManagerWeb(AppManagerGeneric):
    _replacements = [
        ('#name_venv#', 'env'),
    ]
